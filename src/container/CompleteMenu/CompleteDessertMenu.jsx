...
const CompleteDessertMenu = () => {
  ...
  const { global_state: {complete_dessert_menu} } = useContext(GlobalContext);
  useEffect( () => {
    setSelectedMenu(complete_dessert_menu);
  }, [complete_dessert_menu]);

  const category = new Map();

  for (let item of data.dessertMenu)
    {
  ...
  const [selectedMenu, setSelectedMenu] = useState(complete_dessert_menu);
    ...
    <div className="app__completeMenu-wrapper">
      <div className="app__completeMenu flex__center section__padding app__bg-bar" id="complete-menu" ref={completeMenu}>
        ...
        <div className="app__completeMenu_title">
          <SubHeading title="True ingredients, true sweetness, true love!" margin="2rem" />
          <h1 className="headtext__cormorant" id='menu'>Our finest selection of Desserts</h1>
        </div>
        ...
        <div className="app__completeMenu_menu flex__center" id='complete-menu-item' ref={menuList}>
            {data.dessertMenu.map( (dessert, index) => {
                if (dessert.category === selectedMenu)
                  {
                    return (
                      <div key={dessert.category + index} className="app__completeMenu_menu-items">
                        <MenuItem key={dessert.category + dessert.title + index} title={dessert.title} price={dessert.price} tags={dessert.tags} />
                      </div>
                    )
                  }
              } )}
...
export default CompleteDessertMenu;